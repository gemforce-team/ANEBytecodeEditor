...
public:
    ...
    constexpr SmallTrivialVector(std::size_t count, const T& value = T())
        : data(), populated(std::clamp<std::size_t>(count, 0, Size))
    {
    ...
    constexpr bool emplace_back(const T& value) { return push_back(value); }
    ...
    operator SmallTrivialVector<T, NewSize>()
    {
        return SmallTrivialVector<T, NewSize>(data);
    }

    constexpr std::size_t size() const { return populated; }

    constexpr std::size_t capacity() const { return Size; }

    constexpr typename std::array<T, Size>::iterator begin() { return data.begin(); }

    constexpr typename std::array<T, Size>::iterator end() { return data.begin() + size(); }
    ...
    constexpr typename std::array<T, Size>::reverse_iterator rend() { return data.rend(); }

    constexpr typename std::array<T, Size>::const_iterator begin() const { return data.begin(); }
    ...
    constexpr typename std::array<T, Size>::const_iterator cbegin() const { return data.begin(); }
